...
def build_channel_markup(category, file_key=None):
    ...
    # اضافه کردن file_key به callback_data برای فایل تکی
    if file_key:
...
def download_and_send_file(chat_id, user_id, file_key, send_confirmation=True, check_membership=True):
    ...
    # ارسال پیام تأیید برای فایل تکی
...
def send_file_id(message):
    ...
    if file_id:
        # دکمه با داده کوتاه برای جلوگیری از خطا
...
def handle_copy(call):
    # استخراج file_id از متن پیام
    message_text = call.message.text
    if "`" in message_text:
        file_id = message_text.split("`")[1]
        bot.answer_callback_query(call.id, f"✅ File ID کپی شد!", show_alert=True)
    else:
        bot.answer_callback_query(call.id, "❌ خطا در کپی", show_alert=True)
...
def handle_start(message):
    ...
    if len(parts) > 1:
        ...
        if param.startswith('batch_'):
            ...
            # ارسال فایل‌ها بدون بررسی مجدد عضویت
            successful_sends = 0
...
def handle_callback(c):
    if c.data.startswith("check_single_"):
        # برای فایل تکی - فقط همان فایل ارسال شود
        ...
        # برای فایل batch - فقط فایل‌های batch ارسال شوند
        category = c.data.replace("check_batch_", "")
        if is_member(c.from_user.id, category):
            bot.answer_callback_query(c.id, "✅ عضویت شما تأیید شد!", show_alert=True)
            
            # پیدا کردن فایل‌های اصلی batch از متن پیام
            original_files = []
            if "batch_" in c.message.text:
                # استخراج فایل‌های batch از متن
                import re
                batch_match = re.search(r'batch_([a-zA-Z0-9_]+)', c.message.text)
                if batch_match:
                    original_files = batch_match.group(1).split('_')
            
            successful_sends = 0
            for file_key in original_files:
                if file_key in FILE_DATABASE and get_file_category(file_key) == category:

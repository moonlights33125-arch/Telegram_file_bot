...
def build_channel_markup(category, file_key=None):
    ...
    if file_key:
        markup.row(InlineKeyboardButton("✅ بررسی عضویت", callback_data=f"check_single_{category}_{file_key}"))
    else:
...
def download_and_send_file(chat_id, user_id, file_key, send_confirmation=True):
...
def handle_start(message):
    ...
    if len(parts) > 1:
        ...
        if param.startswith('batch_'):
            ...
            for file_key in files:
                if file_key in FILE_DATABASE:
                    if download_and_send_file(message.chat.id, message.from_user.id, file_key, send_confirmation=False):
                        successful_sends += 1
                else:
            ...
            download_and_send_file(message.chat.id, message.from_user.id, param, send_confirmation=True)
...
def handle_callback(c):
    if c.data.startswith("check_single_"):
        ...
        if is_member(c.from_user.id, category):
            ...
            # فقط فایل تکی رو ارسال کن
            if download_and_send_file(c.message.chat.id, c.from_user.id, file_key, send_confirmation=True):
                # حذف پیام عضویت
                try:
        ...
        if is_member(c.from_user.id, category):
            ...
            for key in category_files:
                if download_and_send_file(c.message.chat.id, c.from_user.id, key, send_confirmation=False):
                    successful_sends += 1
                time.sleep(0.5)

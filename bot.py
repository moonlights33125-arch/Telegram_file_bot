...
# ---------- ارسال همه فایل‌های یک دسته ----------
def send_all_files(chat_id, user_id, category):
    # پیدا کردن همه فایل‌های این دسته
    category_files = []
    ...
    if not category_files:
        bot.send_message(chat_id, f"❌ هیچ فایلی در دسته {category} موجود نیست.")
        return
    ...
    if not is_member(user_id, category):
        markup = build_channel_markup(category)
        if markup:
            bot.send_message(chat_id, "❌ برای دریافت فایل‌ها باید ابتدا در چنل‌ها عضو شوید.", reply_markup=markup, parse_mode="HTML")
        return
    
    # ارسال فایل‌ها
    for key in category_files:
        download_and_send_file(chat_id, user_id, key)
        time.sleep(0.5)  # تأخیر بین ارسال‌ها
    
    # پیام تأیید ارسال فایل‌ها
    try:
        bot.send_message(chat_id, "✅ فایل‌ها ارسال شد. (پیام‌ها پس از ۱ دقیقه حذف خواهند شد.لطفا ذخیره کنید.)")
    except Exception:
        pass
...
def handle_start(message):
    ...
    if len(parts) > 1:
        ...
        if param.startswith('batch_'):
            ...
            # پیام تأیید برای batch
            try:
                bot.send_message(message.chat.id, "✅ فایل‌ها ارسال شد. (پیام‌ها پس از ۱ دقیقه حذف خواهند شد.لطفا ذخیره کنید.)")
            except Exception:
                pass
            ...
            # پیام تأیید برای فایل تکی
            try:
                bot.send_message(message.chat.id, "✅ فایل ارسال شد. (پیام پس از ۱ دقیقه حذف خواهد شد.لطفا ذخیره کنید.)")
            except Exception:
                pass
        return
...
def handle_callback(c):
    if c.data.startswith("check_"):
        ...
        if is_member(c.from_user.id, category):
            ...
            for key in category_files:
                download_and_send_file(c.message.chat.id, c.from_user.id, key)
                time.sleep(0.5)
            
            # پیام تأیید ارسال فایل‌ها
            try:
                bot.send_message(c.message.chat.id, "✅ فایل‌ها ارسال شد. (پیام‌ها پس از ۱ دقیقه حذف خواهند شد.لطفا ذخیره کنید.)")
            except Exception:
                pass
        else:
